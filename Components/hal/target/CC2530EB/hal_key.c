...
#define HAL_KEY_S1_IEN      IEN2  /* CPU interrupt mask register */
#define HAL_KEY_S1_IENBIT   BV(1) /* Mask bit for all of Port_2 */
#define HAL_KEY_S1_ICTL     P2IEN /* Port Interrupt Control register */
#define HAL_KEY_S1_ICTLBIT  BV(0) /* P2IENL - P2.0<->P2.3 enable/disable bit */

#define HAL_KEY_S2_IEN      IEN2  /* CPU interrupt mask register */
#define HAL_KEY_S2_IENBIT   BV(1) /* Mask bit for all of Port_2 */
#define HAL_KEY_S2_ICTL     P2IEN /* Port Interrupt Control register */
#define HAL_KEY_S2_ICTLBIT  BV(0) /* P2IENL - P2.0<->P2.3 enable/disable bit */

#define HAL_KEY_S3_IEN      IEN2  /* CPU interrupt mask register */
#define HAL_KEY_S3_IENBIT   BV(1) /* Mask bit for all of Port_2 */
#define HAL_KEY_S3_ICTL     P2IEN /* Port Interrupt Control register */
#define HAL_KEY_S3_ICTLBIT  BV(0) /* P2IENL - P2.0<->P2.3 enable/disable bit */
...
void HalKeyConfig (bool interruptEnable, halKeyCBack_t cback)
{
  ...
  if (Hal_KeyIntEnable)
  {
    ...
    PICTL &= ~(HAL_KEY_A_EDGEBIT);    /* Clear the edge bit */
    /* For falling edge, the bit must be set. */
  #if (HAL_KEY_A_EDGE == HAL_KEY_FALLING_EDGE)
    PICTL |= HAL_KEY_A_EDGEBIT;
  #endif
     ...
     * - Clear any pending interrupt
     */
    HAL_KEY_A_ICTL |= HAL_KEY_A_ICTLBIT;
    HAL_KEY_A_IEN |= HAL_KEY_A_IENBIT;
    HAL_KEY_A_PXIFG = ~(HAL_KEY_A_BIT);    
  ...
  else    /* Interrupts NOT enabled */
  {
    ...
    HAL_KEY_A_ICTL &= ~(HAL_KEY_A_ICTLBIT); /* don't generate interrupt */
    HAL_KEY_A_IEN &= ~(HAL_KEY_A_IENBIT);   /* Clear interrupt enable bit */  
#endif
...
*/
  if (!HAL_PUSH_BUTTON1())//S3
  {
    keys |= HAL_KEY_SW_1; 
  }    

  if (!HAL_PUSH_BUTTON2())//S3
  {
    keys |= HAL_KEY_SW_2; 
  }    
  
  if (!HAL_PUSH_BUTTON3())//S3
  {
    keys |= HAL_KEY_SW_3; 
  }    
  ...
  if (!HAL_PUSH_BUTTON5())//S3
  {
    keys |= HAL_KEY_SW_5; 
  }      
  
  if (!HAL_PUSH_BUTTON6())//S3
  {
    keys |= HAL_KEY_SW_6; 
  }  
